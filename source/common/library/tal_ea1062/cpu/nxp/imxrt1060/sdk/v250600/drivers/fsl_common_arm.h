 ...
 */
static inline status_t EnableIRQ(IRQn_Type _interrupt)
{
    status_t status = kStatus_Success;

    if (NotAvail_IRQn == _interrupt)
    {
        status = kStatus_Fail;
    }

#if defined(FSL_FEATURE_NUMBER_OF_LEVEL1_INT_VECTORS) && (FSL_FEATURE_NUMBER_OF_LEVEL1_INT_VECTORS > 0)
    else if ((int32_t)_interrupt >= (int32_t)FSL_FEATURE_NUMBER_OF_LEVEL1_INT_VECTORS)
    {
#if defined(FSL_FEATURE_IRQSTEER_EXT_INT_MAX_NUM) && (FSL_FEATURE_IRQSTEER_EXT_INT_MAX_NUM > 0) && defined(FSL_FEATURE_IRQSTEER_IRQ_START_INDEX) && (FSL_FEATURE_IRQSTEER_IRQ_START_INDEX > 0)
        int32_t irqsteerInstIdx = (int32_t)((_interrupt + 1 - FSL_FEATURE_IRQSTEER_IRQ_START_INDEX) / FSL_FEATURE_IRQSTEER_EXT_INT_MAX_NUM);

        IRQSTEER_EnableInterrupt(irqsteerInstIdx, _interrupt);
#else
        status = kStatus_Fail;
#endif
    }
    ...
    else
    {
        ...
        GIC_EnableIRQ(_interrupt);
#else
        NVIC_EnableIRQ(_interrupt);
#endif
    }
 ...
 */
static inline status_t DisableIRQ(IRQn_Type _interrupt)
{
    status_t status = kStatus_Success;
    ...
    {
#if defined(FSL_FEATURE_IRQSTEER_EXT_INT_MAX_NUM) && (FSL_FEATURE_IRQSTEER_EXT_INT_MAX_NUM > 0) && defined(FSL_FEATURE_IRQSTEER_IRQ_START_INDEX) && (FSL_FEATURE_IRQSTEER_IRQ_START_INDEX > 0)
        int32_t irqsteerInstIdx = (int32_t)((_interrupt - FSL_FEATURE_IRQSTEER_IRQ_START_INDEX) / FSL_FEATURE_IRQSTEER_EXT_INT_MAX_NUM);

        IRQSTEER_DisableInterrupt(irqsteerInstIdx, _interrupt);
#else
        status = kStatus_Fail;
#endif
    }
    ...
    {
#if defined(__GIC_PRIO_BITS)
        GIC_DisableIRQ(_interrupt);
#else
        NVIC_DisableIRQ(_interrupt);
#endif
    }
 ...
 */
static inline status_t EnableIRQWithPriority(IRQn_Type _interrupt, uint8_t priNum)
{
    status_t status = kStatus_Success;
    ...
    {
#if defined(__GIC_PRIO_BITS)
        GIC_SetPriority(_interrupt, priNum);
        GIC_EnableIRQ(_interrupt);
#else
        NVIC_SetPriority(_interrupt, priNum);
        NVIC_EnableIRQ(_interrupt);
#endif
    }
 ...
 * @retval kStatus_Fail Failed to set the interrupt priority.
 */
static inline status_t IRQ_SetPriority(IRQn_Type _interrupt, uint8_t priNum)
{
    status_t status = kStatus_Success;
 ...
 * @retval kStatus_Fail Failed to set the interrupt priority.
 */
static inline status_t IRQ_ClearPendingIRQ(IRQn_Type _interrupt)
{
    status_t status = kStatus_Success;
    ...
    {
#if defined(__GIC_PRIO_BITS)
        GIC_ClearPendingIRQ(_interrupt);
#else
        NVIC_ClearPendingIRQ(_interrupt);
#endif
    }
